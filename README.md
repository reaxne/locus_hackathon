...
Серверный AI-бюджет составляет 60 секунд; прокси ждёт 90 секунд, браузер — 95 секунд,
поэтому транспорт не обрывает корректное серверное завершение. Если все бесплатные
модели не ответили, NDJSON завершается событием `error`, экран загрузки закрывается,
базовый маршрут или предыдущий результат остаётся видимым, а кнопка предлагает
«Повторить с ИИ». Событие `completed` принимается только для проверенного результата.
